...
|---|---|---|---|
| Payment gateway (Razorpay) | **CODE COMPLETE, PARTLY VERIFIED IN TEST MODE** | Production keys and a webhook secret must be set on Render; a real successful test payment still has to be completed by a person | Real SDK integration (Orders API, signature verification, webhooks, refunds) built and unit-tested in Phases 1–2. **Test-mode keys were configured locally on 2026-09-20** (gitignored env files) and used to verify against Razorpay's real test servers: order creation, the checkout window opening with the correct amount, cancel, decline (simulated event), and rejection of a forged signature with no order created. `RAZORPAY_*` remain **unset on the live Render deployment** (`render.yaml` declares them `sync: false`). Live-verified: COD end-to-end, and the "gateway not configured" failure path (real 400 response, reservation correctly released). **Not verified: an actual captured card/UPI/net-banking test payment, webhook delivery, and refunds.** Do not upgrade this row to VERIFIED until a person completes a real Checkout.js payment in test mode and the webhook and a refund are observed. |
