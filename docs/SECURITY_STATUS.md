...
| Dependency vulnerabilities (`npm audit`) | ✅ 0 VULNERABILITIES (2026-09-19) | Clean after P0-B (`deepmerge-ts` override); regressed to 6 high findings when four `multer <=2.2.0` advisories were published; fixed by a `multer@2.4.0` direct dependency + override, verified with real multipart uploads. Re-run `npm audit` regularly — new advisories appear without any code change |
| Demo accounts with public passwords | ✅ FIXED IN CODE (2026-09-19), ⚠️ PRODUCTION DB UNVERIFIED | `seed.ts` no longer creates known-password accounts when `NODE_ENV=production`; `SEED_LOCK_DEMO_ACCOUNTS=true` locks ones that already exist. Whether the live database contains them is not known — see `docs/MANUAL_SETUP_GUIDE.md` §0.2 |
