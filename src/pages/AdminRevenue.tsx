...
export default function AdminRevenue() {
  ...
  const { series, totals } = useRevenueAnalytics(granularity, windowDays);

  return (
    <div>
      <PageHeader title="Revenue Analytics" description="Gross/net revenue, discounts, and shipping revenue." />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <ExportButton data={series} filename={`revenue-${granularity}`} />
        </div>
      </div>
      ...
      <LineChartWidget
        ...
        xKey="label"
        height={340}
        description={`${granularity} gross and net revenue over the selected date range, ${series.length} data points.`}
      />
    </div>
  );
}
