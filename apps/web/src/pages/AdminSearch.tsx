...
import { useSearchAnalytics, useRealAdminApi } from '@/hooks/useAdminAnalytics';
  ...
  return (
    <div>
      <PageHeader
        ...
        description={
          useRealAdminApi
            ? "Trending search terms from the store's real search logs. Per-term counts, no-result searches, and click-through rate aren't tracked server-side yet."
            : 'Top and trending searches combine a deterministic mock baseline with real terms searched in this session. No-result searches and click-through rate are computed entirely from real logged search events.'
        }
      />
      ...
      {useRealAdminApi ? (
        <div className="mb-10">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-lg font-semibold text-heading">Trending searches</h2>
            <ExportButton data={topSearches} filename="trending-searches" />
          </div>
          {topSearches.length === 0 ? (
            <p className="text-sm text-ink-soft py-8 text-center">No trending searches yet.</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {topSearches.map((s) => (
                <span key={s.term} className="px-3 py-1.5 rounded-full border border-stone-dark text-sm text-ink-soft">
                  {s.term}
                </span>
              ))}
            </div>
