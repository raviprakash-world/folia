...
describe('ShippingService.estimate', () => {
  ...
  it('falls back to the flat-rate heuristic when Shiprocket is not configured (throws)', async () => {
    ...
    const result = await service.estimate('560001', 500);
    expect(result).toEqual({ cost: 79, etaDays: '2–4 business days' });
  });
  ...
  it('is free once subtotal reaches the ₹999 threshold (inclusive) via the fallback heuristic, regardless of region', async () => {
    const { service } = createDeps();
    await expect(service.estimate('902100', 999)).resolves.toEqual({
      cost: 0,
      etaDays: '3–5 business days',
    });
    await expect(service.estimate('100010', 1500)).resolves.toEqual({
      cost: 0,
      etaDays: '3–5 business days',
    });
  });
  ...
  it('far-region fallback rate below the threshold', async () => {
    const { service } = createDeps();
    const result = await service.estimate('110001', 500);
    expect(result).toEqual({ cost: 129, etaDays: '4–6 business days' });
  });
});
