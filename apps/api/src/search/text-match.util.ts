/** Ported directly from apps/web/src/utils/textMatch.ts — same match-quality tiers, same real Levenshtein edit-distance did-you-mean logic. */
...
const matchQualityWeight: Record<MatchQuality, number> = {
  exact: 3,
  prefix: 2,
  contains: 1,
  none: 0,
...
/** Simple relevance sort for small datasets (categories, collections) — exact > prefix > contains. */
export function sortByRelevance<T>(
  ...
  return [...items]
    .map((item) => ({
      item,
      weight: matchQualityWeight[getMatchQuality(getText(item), query)],
    }))
    .filter((x) => x.weight > 0)
...
/** "Did you mean?" — finds the closest term to `query` among `candidates` within a small edit-distance threshold. Null if nothing is close enough, or if the query already matches something. */
