...
// Reverse of the above — used by admin write operations (AdminProductsService).
const badgeFromPublic: Record<
  'New' | 'Sale' | 'Bestseller' | 'Low stock',
  ProductBadge
> = {
  New: 'NEW',
...
const careLevelFromPublic: Record<'Easy' | 'Moderate' | 'Advanced', CareLevel> =
  {
    Easy: 'EASY',
    Moderate: 'MODERATE',
    Advanced: 'ADVANCED',
...
export function badgeToDb(
  badge: 'New' | 'Sale' | 'Bestseller' | 'Low stock' | undefined,
): ProductBadge | null {
  return badge ? badgeFromPublic[badge] : null;
}

export function careLevelToDb(
  careLevel: 'Easy' | 'Moderate' | 'Advanced' | undefined,
): CareLevel | null {
  return careLevel ? careLevelFromPublic[careLevel] : null;
}
